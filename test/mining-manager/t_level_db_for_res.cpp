...
void test2()
{
    ...
    if(levelDBClusteringData.init(HOME_STR2))
    {
        ...
        vector<ClusteringInfo> infos;
        levelDBClusteringData.loadClusteringInfos(infos);
        for(vector<ClusteringInfo>::iterator iter = infos.begin(); iter != infos.end(); iter++)
        {
             cout<<iter->clusteringDocNum<< "pow size:"<<iter->clusteringPow.size()<<endl;
        }
