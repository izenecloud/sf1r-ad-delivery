 ...
 * @date Created <2014-06-23 20:21:43>
 *
...
#include "controllers/SlimController.h"
#include "controllers/AdController.h"
...
void initializeDriverRouter(::izenelib::driver::Router& router, IService* service, bool enableTest)
{
    ...
    {
        SlimController slim;
        const std::string controllerName("slim");
        typedef ::izenelib::driver::ActionHandler<SlimController> handler_type;
        typedef std::auto_ptr<handler_type> handler_ptr;

        handler_ptr recommendHandler(
            new handler_type(
                slim,
                &SlimController::recommend
            )
        );
...

    {
        ...
        handler_ptr del_ad_bid_phraseHandler(
            new handler_type(
                ad,
                &AdController::del_ad_bid_phrase
            )
        );
        ...
        router.map(
            controllerName,
            "del_ad_bid_phrase",
            del_ad_bid_phraseHandler.get()
        );
        del_ad_bid_phraseHandler.release();

        handler_ptr set_ad_campaign_budgetHandler(
            new handler_type(
                ad,
                &AdController::set_ad_campaign_budget
            )
        );
        ...
        router.map(
            ...
            set_ad_campaign_budgetHandler.get()
        );
        set_ad_campaign_budgetHandler.release();

        handler_ptr update_ad_bid_phraseHandler(
            new handler_type(
                ad,
                &AdController::update_ad_bid_phrase
            )
        );
        ...
        router.map(
            ...
            update_ad_bid_phraseHandler.get()
        );
        update_ad_bid_phraseHandler.release();

        handler_ptr update_online_statusHandler(
            new handler_type(
                ad,
                &AdController::update_online_status
            )
        );
        ...
        router.map(
            ...
            update_online_statusHandler.get()
        );
        update_online_statusHandler.release();
    }
